...
def omit_unsubscribed(odata, unsubscribers):
  unsubscribersList = unsubscribers['StudentID'].tolist()
  studentsList = odata['StudentID'].tolist()
  row_numbers= []

  #makes a list of all the row numbers to get rid of in the original file
  for iD in unsubscribersList:
    for i in range(len(studentsList)):
      if (iD == studentsList[i]):
        row_numbers.append(i)

  #deletes all the rows necessary
  for row in row_numbers:
    odata.drop(row,axis=0,inplace=True)
  return(odata)
...
def enroll(pre,act,disc,level,reading,math,odata):
  enrolldata = klevel(level,reading,math,odata)
  if(act and disc and pre):
    return enrolldata
    #ask hema about the Z factor, ommiting all students with a Z status (anyone who doesn't want to be part of the emailing list)
  elif (pre and act) :
    enrollSorted = enrolldata.loc[(enrolldata['ST1']=='C') | (enrolldata['ST1']=='C ')  | (enrolldata['ST1']=='IT') | (enrolldata['ST1']=='IT ') | (enrolldata['ST1']=='R') | (enrolldata['ST1']=='R ') | (enrolldata['ST1']=='N') | (enrolldata['ST1']=='N ') | (enrolldata['ST1']=='P') | (enrolldata['ST1']=='P ')]
    return enrollSorted
  elif (pre and disc):
    enrollSorted = enrolldata.loc[(enrolldata['ST1']=='D') | (enrolldata['ST1']=='D ') | (enrolldata['ST1']=='OT') | (enrolldata['ST1']=='OT ') | (enrolldata['ST1']=='A') | (enrolldata['ST1']=='A ') | (enrolldata['ST1']=='P') | (enrolldata['ST1']=='P ')]
    return enrollSorted
  elif (act and disc):
    enrollSorted = enrolldata.loc[(enrolldata['ST1']=='D') | (enrolldata['ST1']=='D ') | (enrolldata['ST1']=='OT') | (enrolldata['ST1']=='OT ') | (enrolldata['ST1']=='A') | (enrolldata['ST1']=='A ') | (enrolldata['ST1']=='C') | (enrolldata['ST1']=='C ')  | (enrolldata['ST1']=='IT') | (enrolldata['ST1']=='IT ') | (enrolldata['ST1']=='R') | (enrolldata['ST1']=='R ') | (enrolldata['ST1']=='N') | (enrolldata['ST1']=='N ')]
  elif (pre):
    enrollSorted = enrolldata.loc[(enrolldata['ST1']=='P') | (enrolldata['ST1']=='P ')]
    return enrollSorted
  elif (act):
...
def grade(starting,ending,pre,act,disc,level,reading,math,odata):
  gradedata = enroll(pre,act,disc,level,reading,math,odata)
  #converting input
  if (starting=="K"):
    s = 0
  elif (starting=="PK1"):
  ...
  for i in range(len(oldgrades)):
    if (enrollstatus[i] =="D" or enrollstatus[i] =="D " or enrollstatus[i] =="A " or enrollstatus[i] =="OT " or enrollstatus[i] =="A" or enrollstatus[i] =="OT" or enrollstatus[i] =="P" or enrollstatus[i] =="P "):
...
def birthday(s,e,starting,ending,pre,act,disc,level,reading,math,odata):
  birthdaydata = grade(starting,ending,pre,act,disc,level,reading,math,odata)
